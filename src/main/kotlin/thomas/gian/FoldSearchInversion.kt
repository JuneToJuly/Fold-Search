...
class FoldSearchInversion : FoldSearchBase() {
    ...
    override fun actionPerformed(event: AnActionEvent) {
        ...
        val foldSearchTitle = "Fold Search Inversion"
        var lastSearch = ""
            ...
            JBPopupFactory.getInstance().createComponentPopupBuilder(topLevelPanel, foldSearchField)
                ...
                .setCancelCallback {
                    ...
                    if (content?.component is FoldSearchHistoryWindow) {
                        ...
                        lastSearch = foldSearchHistoryWindow.backStack.peek().second;
        ...
        foldSearchField.addDocumentListener(object :
            ...
            override fun documentChanged(event: com.intellij.openapi.editor.event.DocumentEvent) {
                ...
                val searchText = startingString
                val inversionText = foldSearchField.text
                var ignoreCase = true
                ...
                selectedTextEditor?.foldingModel?.runBatchFoldingOperation {
                    ...
                    fileEditorManager.selectedTextEditor?.document?.let { doc ->
                        lines.forEach {
                            ...
                            var inversionIndex = fullLine.indexOf(inversionText);
                            inversionIndex =  if(inversionIndex == -1) 0 else inversionIndex
                            val lineStart = doc.getLineStartOffset(lineIndex) + inversionIndex
                            FoldSearchUtil.createFoldSearchRegion(selectedTextEditor, currentOffset, lineStart)
                            currentOffset = if(inversionIndex == 0) lineStart + fullLine.length + 1 else lineStart + inversionText.length + 1
                        }
